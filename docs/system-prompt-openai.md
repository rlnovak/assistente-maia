# System Prompt — OpenAI (GPT)

> Estilo: seções `##` com listas enumeradas e bullets. GPT segue melhor regras explícitas do que diretrizes em prosa contínua.
...
`maia-backend/app/llm/prompts/system_openai.py`

## Notas para otimização

- `## Identidade` e `## Escopo` equivalem às tags `<persona>` e `<escopo>` do Claude
- Guardrails são numerados — GPT segue listas enumeradas com mais fidelidade que prosa
- `## Postura emocional` é nova seção em relação ao prompt anterior — cobre validação vs. validação de prática
- `## Encaminhamento médico` é nova seção — lista explícita de sintomas que requerem médico
- Ao testar no ChatGPT: cole como "System" e use `<contexto_rag>texto de exemplo</contexto_rag>` na mensagem do usuário
- Mudanças no prompt OpenAI **não afetam** o prompt Anthropic — são arquivos independentes
