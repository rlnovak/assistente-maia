...
> Estilo: seções markdown com `##`, listas numeradas e bullets. GPT segue melhor regras enumeradas explicitamente do que diretrizes em prosa.
...
- A seção `## Identidade` equivale à `<persona>` do Claude — é onde a voz é definida
- Os guardrails são numerados intencionalmente — GPT segue listas enumeradas com mais fidelidade que prosa
- Ao testar no ChatGPT: cole o prompt como "System" e use `<contexto_rag>texto de exemplo</contexto_rag>` na mensagem do usuário
